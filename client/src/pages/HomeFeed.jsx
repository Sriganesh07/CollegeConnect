...
import {
  ...
  FiChevronLeft, FiChevronRight, FiCamera, FiBarChart2, FiEye,
} from 'react-icons/fi';
...
// ── NEW: import the SuggestedPeople widget ──
...
function PostCard({ post: init, currentUser, onDelete }) {
  ...
  // ── Views: read from localStorage, increment once per post per session
  const [views, setViews] = useState(() => {
    try {
      const stored = JSON.parse(localStorage.getItem('cc_post_views') || '{}');
      return stored[init.id] || init.views_count || 0;
    } catch { return init.views_count || 0; }
  });
  ...
  // Increment view once when the card scrolls into view
  useEffect(() => {
      ...
      if (entry.isIntersecting && !viewedRef.current) {
        ...
        setViews(v => {
          ...
          try {
            const stored = JSON.parse(localStorage.getItem('cc_post_views') || '{}');
            stored[init.id] = next;
            localStorage.setItem('cc_post_views', JSON.stringify(stored));
          } catch {}
          return next;
        });
        // Optionally tell backend (fire-and-forget)
        postAPI.view?.(init.id)?.catch(() => {});
        obs.disconnect();
      }
    }, { threshold: 0.6 });
    obs.observe(el);
  ...
  return (
    <article className="post-card" ref={cardRef}>
      ...
      {/* ── Views row ── */}
      <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 16px 0',color:'var(--text3)'}}>
        <FiEye size={12}/>
        <span style={{fontSize:11.5,fontWeight:500}}>{fmtCount(views)} views</span>
      </div>

      {/* ── Stats row (likes · comments · saves) ── */}
      <div className="post-stats">
        <span style={{display:'flex',alignItems:'center',gap:4}}>
          <span style={{fontSize:13}}>{myReaction || (post.isLiked ? '❤️' : '🤍')}</span>
          <span>{fmtCount(post.likes_count)}</span>
          <span style={{color:'var(--border2)',margin:'0 2px'}}>·</span>
          <FiMessageCircle size={12} style={{color:'var(--text3)'}}/>
          <span>{fmtCount(post.comments_count)}</span>
          <span style={{color:'var(--border2)',margin:'0 2px'}}>·</span>
          <FiBookmark size={12} style={{color: post.isSaved ? '#f59e0b' : 'var(--text3)', fill: post.isSaved ? '#f59e0b' : 'none'}}/>
          <span>{fmtCount(post.saves_count || 0)}</span>
        </span>
      </div>
      ...
      <div className="post-actions">
        ...
        <div style={{flex:1,position:'relative'}}>
          ...
          >
            <span style={{fontSize:15,lineHeight:1}}>{myReaction||(post.isLiked?'❤️':'🤍')}</span>
            <span>Like</span>
            <span style={{
              fontSize:11,fontWeight:700,
              background: post.isLiked ? 'rgba(244,63,94,0.15)' : 'var(--bg2)',
              color: post.isLiked ? 'var(--rose)' : 'var(--text3)',
              padding:'1px 6px',borderRadius:20,minWidth:20,textAlign:'center',
            }}>{fmtCount(post.likes_count)}</span>
          </button>
          {showReact && (
        ...
        {/* Comment */}
        <button className="post-action-btn" style={{gap:5}} onClick={openCommentSection}>
          <FiMessageCircle size={15}/>
          <span>Comment</span>
          <span style={{
            fontSize:11,fontWeight:700,
            background: openComments ? 'rgba(37,99,235,0.12)' : 'var(--bg2)',
            color: openComments ? 'var(--accent)' : 'var(--text3)',
            padding:'1px 6px',borderRadius:20,minWidth:20,textAlign:'center',
          }}>{fmtCount(post.comments_count)}</span>
        </button>

        {/* Share */}
        <button className="post-action-btn" onClick={()=>navigator.clipboard.writeText(location.origin)}>
          <FiShare2 size={15}/><span>Share</span>
        </button>

        {/* Save/Bookmark */}
        <button className={`post-action-btn save-btn ${post.isSaved?'saved':''}`} style={{gap:5}} onClick={toggleSave}>
          <FiBookmark size={15} style={post.isSaved?{fill:'#f59e0b',color:'#f59e0b'}:{}}/>
          <span style={{
            fontSize:11,fontWeight:700,
            background: post.isSaved ? 'rgba(245,158,11,0.12)' : 'var(--bg2)',
            color: post.isSaved ? '#f59e0b' : 'var(--text3)',
            padding:'1px 6px',borderRadius:20,minWidth:20,textAlign:'center',
          }}>{fmtCount(post.saves_count || 0)}</span>
        </button>
      </div>
  ...
  return (
    <div className="feed-root">
      ...
      <aside className="feed-aside">
        ...
        {/* ── NEW: People You May Know (dynamic, from real users) ── */}
        <SuggestedPeople />
...
/* ── SuggestCard (kept for backwards compatibility, no longer used in aside) */
